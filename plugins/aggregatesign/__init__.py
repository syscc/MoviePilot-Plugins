...
class AggregateSign(_PluginBase):
    ...
    def _auto_login(self) -> Tuple[bool, str]:
        ...
        self.update_config(self._build_config())
        logger.info(
            f"{self._current_site_name}自动登录成功，account={self._current_account_name}, "
            f"cookie_len={len(self._cookie)}, storage_state_len={len(self._storage_state)}"
        )
        return True, message
    ...
    def _migrate_base_url(site_key: str, base_url: Any) -> str:
        ...
        if site_key == "hdhive" and value in {
            "https://hdhive.com",
            "https://www.hdhive.com",
        }:
            return "https://re0.me"
        return value
                        ...
                        "content": [{
                            ...
                            "content": [{
                                ...
                                "props": {
                                    "type": "info",
                                    "variant": "tonal",
                                    "text": "使用说明：只使用上方 JSON 多账号配置。聚影可通过 base_url 选择国内 https://jying.top 或国外 https://www.jying.top，不填默认使用国外入口。site 支持 juying、dian115 和 hdhive；三个站点都可填写 username/password 自动登录。影巢默认使用 https://re0.me，登录态绑定浏览器环境，推荐使用账号密码，插件会保存完整浏览器状态。methods 支持 normal；dian115 还支持 lucky，hdhive 还支持 gamble（风险签到可能扣积分，不建议默认开启）。",
                                },
                            }],
                        }],
                    },
